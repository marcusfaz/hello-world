...
/* imagery: the real Generation III Emerald battle sprites, embedded as data URIs */
const monSrc = n => (typeof IMG !== 'undefined' && IMG.mon[n]) || null;
...
function itemArt(kind, name){
  ...
  let src = null;
  const mach = /^(TM|HM)(\d+)$/.exec(kind);
  if (mach){
    const row = (typeof MACHINES !== 'undefined') && MACHINES.find(r => r[0] === kind);
    if (row) src = IMG.itm[mach[1].toLowerCase() + ':' + row[2].toLowerCase()];
  }
  if (!src) src = IMG.itm[name] || monSrc(name);
  return src ? '<img class="isp" src="' + src + '" alt="" loading="lazy" aria-hidden="true">' : '';
}
const tpills = a => (a || []).map(tpill).join(' ');

/* ---- Gen 3: damage class is a property of the TYPE, not the move ---- */
const PHYSICAL = ['Normal','Fighting','Flying','Ground','Rock','Bug','Ghost','Poison','Steel'];
const SPECIAL  = ['Fire','Water','Grass','Electric','Psychic','Ice','Dragon','Dark'];
const dclass = t => PHYSICAL.indexOf(t) !== -1 ? 'Physical' : 'Special';
const dcpill = t => '<span class="dc ' + (dclass(t) === 'Physical' ? 'phy' : 'spe') + '">' +
  (dclass(t) === 'Physical' ? 'Phys' : 'Spec') + '</span>';
...
function renderBlock(b, stageId){
  ...
  switch (kind){
    ...
    case 'ver':  return '<div class="note ver"><span class="lbl">Emerald differs from Ruby &amp; Sapphire here</span><p>' + v + '</p></div>';
    case 'html': return v;
    ...
    case 'catch': return '<div class="grid">' + v.map(c =>
        ...
        '<div class="nm">' + esc(c.n) + ' ' + tpills(c.t) + (c.lg ? ' <span class="exc">' + esc(c.lg === true ? 'Emerald only' : c.lg) + '</span>' : '') + '</div>' +
